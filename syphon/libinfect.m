...
static bool infect_in_safe_boot(void) {
    int sb = 0;
    size_t n = sizeof(sb);
...
static int spawn_with_env(int (*spawn_fn)(pid_t *, const char *,
  ...
  if (infect_in_safe_boot()) {
    LogToFile("ammonia: safe boot, not inserting opener for '%s'\n",
              path ? path : "(null)");
    goto Spawn;
  }
  ...
  if (strcmp(path, "/usr/libexec/xpcproxy") == 0) {
    if (!disable_xpcproxy_injection) {
      playground = envbuf_setenv(playground, "DYLD_INSERT_LIBRARIES",
                                 SupportFolderP "libinject.dylib");
    }
...
void __attribute__((constructor)) Infect(void) {
  ...
  if (infect_in_safe_boot()) {
    LogToFile("ammonia: safe boot, infect idle\n");
    return;
  }
