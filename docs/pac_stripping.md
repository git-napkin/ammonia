...
3. If the target is an `.app` bundle, it is copied to `/tmp/RuntimeApplications/` to avoid modifying the original (reused until the source bundle is newer). Non-bundles are spawned as-is.
